...
export const fetchTranscript = async (url: string): Promise<TranscriptResult> => {
    ...
    if (isYouTube) {
        throw new Error("YouTube transcript fetching requires a specialized Actor not yet configured. Please provide a URL to a text transcript or blog post.");
    }

    // Use tilde instead of slash for username~actorname convention in API v2
