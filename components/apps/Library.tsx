...
const Library: React.FC<LibraryProps> = ({ onSelectIdea }) => {
  ...
  useEffect(() => {
    ...
    // Poll for changes in case multiple windows are open
        ...
        {ideas.length === 0 ? (
          ...
          ideas.map(idea => (
            <div 
              ...
              className="w-full bg-slate-900 border border-slate-800 p-4 rounded-xl hover:border-blue-500/50 hover:bg-slate-800/50 transition-all cursor-pointer group shadow-sm active:scale-[0.99]"
            >
              <div className="flex items-start justify-between mb-2">
                ...
                <span className={`text-[7px] font-black uppercase tracking-tighter px-2 py-0.5 rounded ${idea.status === 'digested' ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-500/20 text-slate-400'}`}>
                  {idea.status}
                </span>
              </div>
              <p className="text-[12px] font-semibold text-slate-300 line-clamp-2 leading-relaxed italic mb-3">
                "{idea.content}"
              </p>
